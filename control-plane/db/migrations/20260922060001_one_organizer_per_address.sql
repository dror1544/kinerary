-- rollback: compatible — a data repair (identity backfill, trip_memberships/telegram_organizer_links reassignment, duplicate accounts marked deleted); no column or table changes shape, and it is written to be safe to re-run
--
-- ONE ADDRESS, ONE ORGANIZER — and the repair of the rows that say otherwise.
--
-- 20260922060000 introduced an account an operator could create for somebody else, and
...
-- refused. Production has none of these rows; it never ran 20260922060000.
