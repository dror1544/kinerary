...
describe("document intake through the interview", { skip: SKIP ? "no CONTROL_PLANE_TEST_DATABASE_URL" : false }, () => {
  ...
  test("confirming records which documents the version was built from, and none of their text", async () => {
    await withFlow({ store: true }, async (flow) => {
      ...
      // organizer_identity is satisfied only by a name that resolves to
      // exactly one traveler — plain filler text resolves to nobody.
      answers.travelers ??= { kind: "structured", schema_version: INTAKE_SCHEMA_VERSION, data: [{ name: "Dana" }] };
      ...
      answers.organizer_identity = { kind: "text", schema_version: INTAKE_SCHEMA_VERSION, text: "Dana" };
