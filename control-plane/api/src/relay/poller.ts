...
async function runDocumentPath(
  ...
  try {
    await readDocumentsInto(deps, deps.modelRunner, burst, tripId, registered, language, say, ask, commit, log);
  } finally {
...
async function readDocumentsInto(
  ...
  // and the reading above already takes minutes — running it before the recap
  // left an organizer watching nothing happen for seven minutes on 2026-09-12,
  // having been told the document was being read.
  //
  ...
  await foldItineraryFromDocument(deps, burst, usable.map((e) => e.text).join("\n\n"), log, say, language);
}
