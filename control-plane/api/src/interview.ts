...
export const INTAKE_QUESTIONS: readonly IntakeQuestion[] = [
  ...
  },
  {
    id: "phases",
    type: "structured",
    // NO `days` HERE, deliberately.
    //
    // This question is answered by the survey pass — one call that reads a
    // document for twenty answers at once. Asking it to transcribe a
    // day-by-day as well doubled what that call had to produce, and on
    // 2026-09-12 a four-page itinerary that had been extracted in about three
    // minutes stopped finishing at all: two 240-second attempts, 481 seconds,
    // nothing captured, and the organizer asked for everything by hand.
    //
    // `extract_itinerary` is the pass built for the itinerary, and it runs
    // straight after this one (`foldItineraryFromDocument`) with its own prompt
    // and its own schema. `_derive_phases` still reads `days` off this answer —
    // that is how the folded result gets in — it is simply not this call's job
    // to produce them.
