...
export function applyProposals(
  ...
  const ownMerits = (p: ProposedAnswer): { reason: RejectReason; detail?: string } | null => {
    ...
    // Held and structured: reconciled below rather than refused here.
    const reconcilable = ctx.held?.[p.questionId] !== undefined;
    if (answered.has(p.questionId) && !reconcilable) return { reason: "ALREADY_ANSWERED" };
    if (!outstanding.has(p.questionId) && !reconcilable) return { reason: "NOT_OUTSTANDING" };
  ...
  const decideStructured = (parts: ProposedAnswer[]) => {
    ...
    accepted.push({ questionId, answer: alone.answer, proposal: primary });
  };
  ...
  proposals.forEach((proposal, i) => {
    ...
    if (answered.has(proposal.questionId)) {
      // A text or a choice cannot be merged, only replaced — so it never is.
      // But a document that credibly says something DIFFERENT is worth a
      // question, where one that is unsure or cannot quote itself is not.
