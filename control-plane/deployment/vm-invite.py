...
ENV = os.environ
DEPLOY_ENV = Path(ENV.get("KINERARY_INVITE_ENV", "/opt/kinerary-deploy/vm.env"))
SECRETS_DIR = Path(ENV.get("KINERARY_SECRETS_DIR", "/opt/kinerary/control-plane/deployment/.local-secrets"))
...
def bot_username(facts: Dict[str, str]) -> str:
    ...
    """
    token_file = facts.get("TELEGRAM_BOT_TOKEN_FILE") or str(SECRETS_DIR / "telegram_creds")
