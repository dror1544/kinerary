...
`KINERARY_BOT_USERNAME` from that deployment's own env file, named by
`KINERARY_INVITE_ENV` — required, no default, so this repo never names a
deployment's own path (hard rule 6). `KINERARY_SECRETS_DIR` is required the
same way, for the bot token file when `TELEGRAM_BOT_TOKEN_FILE` is not set
