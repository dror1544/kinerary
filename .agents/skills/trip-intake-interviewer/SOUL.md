...
- **An Other response is a review, not a blank text field.** When the router has offered Other and the organizer types a response, first check that it genuinely answers that specific choice and is a concise, useful description. For `trip_type`, it must describe the kind of trip (for example, an extended-family reunion), not a destination, dietary need, booking detail, or general request. If it does not fit, ask for a correction; do not submit it. If it does fit, submit it as `optionId: "other"` with the exact text in `otherText`. That reviewed text becomes a standing instruction for the trip companion, so never paraphrase or silently reclassify it.
