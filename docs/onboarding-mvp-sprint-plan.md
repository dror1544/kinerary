...
As of 2026-08-29 every item below is `built` except two low-visual residuals
(`follow-on`): venue **ticket-site links** and **prose anchor extraction** —
both need either a runtime web search or new render surface for marginal gain.
The `separate build` (site live-plan enrichment worker) stays out of 4.5.
...
| Venue Maps/Waze links per place | `built` | `enrichment._add_phase_nav` templates `accommodation.maps` / `accommodation.waze` from the resolved coordinates (`setdefault`, so hand-authored wins). Per-day-item and per-booking-row links, and the **ticket-site lookup**, are the residual — see next row. |
| Venue **ticket-site links** | `follow-on` | A link to an attraction's official ticket/booking site where advance booking is needed. Needs a runtime web lookup (same shape as the consular search) or a `url` field captured by `extract_itinerary` from the source document; the site's config `days[].items[]` render (`renderDays`) would also need to learn a per-item URL. Deferred — low visual weight, real new surface. |
