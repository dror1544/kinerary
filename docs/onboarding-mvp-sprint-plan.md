...
| Accommodation location (Maps/Waze/address) | `built` | `enrich_config` geocodes the **hotel name** (not the city) when a phase has an accommodation, anchors `phase.mapStop` there, and `_add_phase_nav` templates `accommodation.maps` / `accommodation.waze` + `accommodation.address` (Nominatim `display_name`) from those coordinates. Falls back to the city centre only when the hotel won't resolve. Matches the hand-built trips (`kinerary-deploy/trips/los-angeles-hawaii-vegas-2026` etc.: `accommodation.{maps,waze,address}` at the hotel's coords). |
| Per-venue location + **ticket/official links** | `built` | `extract_itinerary` returns `venues[]` per phase — the notable/bookable places the source document names, each with a `url` **only if the document itself prints one** (never guessed). `transformer._normalise_venues` projects `phases[].venues[]` = `[{id, name:{he,en}, url?, area?}]`; `enrichment._enrich_venues` geocodes each and adds `maps` / `waze`. The site renders a card per venue (rating + 🗺️/🔵/🎫 buttons) — `loadRatings` now renders `VENUES[phase.id]` for config-driven phases, which was previously only wired for the removed static HTML sections. Mirrors the hand-built `venues[]` (`{id, name, name_he, url, area}`) in `kinerary-deploy/trips/japan-2025`. |
