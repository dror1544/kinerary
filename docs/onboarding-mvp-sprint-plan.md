...
| Per-phase packing lists | `separate build` — **BUILT (2026-09-23)** | Sourced from Sprint 6 Track 1a item 3 (`docs/sprint6-tracks.md`, #162, PR #165, merge `20f7419`), not part of 4.5's original scope, but the same "fill a thin provisioned config" purpose as the rows above. `transformer._derive_phase_packing` emits `phase.packing` — `[{he,en} category, {he,en} item]` pairs, the shape `readiness.tsx` reads — from a coarse season bucket (`_season_bucket`: cold / rainy / hot / moderate) of the trip destination's hemisphere crossed with the phase's **start month**, mapped to a small fixed bilingual table (`_PACKING_ITEMS_BY_SEASON`). Deterministic: no network, no model. **Coverage is deliberately coarse, not complete.** The hemisphere is per **trip** (the typed destination), not per phase, and resolves through `_country_keys()` / `_COUNTRY_ALIASES` to a list of eleven southern countries (`_KNOWN_COUNTRY_HEMISPHERE`); everything unresolved defaults north. Its remaining coarseness is #167 and stays **OPEN**: a city-only destination ("Sydney") resolves north; a multi-country string is decided by its last comma-separated segment only ("Spain, Chile" resolves south, "Chile, Spain" north; "and"-joined lists are not split and resolve north); a Hebrew geresh variant of Chile/Fiji does not match and resolves north. No climate beyond the season bucket (no monsoon calendars, no desert vs rainforest; spring is "rainy" and autumn "moderate" in both hemispheres), no live weather, and a multi-month phase is bucketed on its start month alone. A phase with no destination or no start date gets nothing rather than a guessed season; trip-level `config.packing_general` is untouched and keeps its own frontend fallback. |
