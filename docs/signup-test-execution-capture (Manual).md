...
| 13 | Verify the English spelling of non-English traveller names | **Fixed** | 2026-09-03, PR #34. The transformer already honoured `name_en`/`family_en`; nothing ever asked for them, so they appeared only if volunteered. The `travelers` prompt and its field guidance now ask — once for the whole list, not name by name. `username` derives from the Latin spelling, so without it the site shows people something they do not recognise. Guidance takes the organizer's transliteration as given rather than standardising it. **Refined 2026-09-05 after run 13**: asking was the wrong half of the fix. Having listed five people in Hebrew, the organizer was asked to write all five names again in English — the same information, typed twice. The interviewer now transliterates itself, submits that as `name_en`/`family_en`, and shows the spellings for correction rather than requesting them; answers overwrite, so a correction arriving later is a plain re-submit. A spelling the organizer gives still wins over one we chose. |
| 14 | Document turns time out and retry, surfacing to the user as repeated interview questions | **Partial** | → **Sprint 5, Track 6.** Run 12 measured a real document turn running over a minute before its first write; the 30s floor reclaimed it and Hermes's next two tool calls returned `404 NOT_FOUND`. `DOCUMENT_FLOOR_SECONDS` (2026-09-05, migration `0040`) widens a media turn to 90s — its own commit calls this "a wider guess, not a fix". **Root cause still open:** nothing yet lets the control plane tell "still working" from "stuck". Directive (Dror, 2026-09-05): fix the turn semantics and make retries idempotent — not a larger timeout. Onboarding-reliability; resolve before scaling live onboarding. |
| 15 | Router should own question progression, with lightweight answer-completeness validation | **Partial** | → **Sprint 5, Track 8**, built 2026-09-05 (`6205b2d`). `routerOwned` gives the router four questions (`trip_type`, `bot_gender`, `bot_tone`, `bot_proactive`) outright; `checkComplete` rejects substantively empty answers with `INCOMPLETE_ANSWER` and writes nothing. **Gap open:** SOUL was not told, so the agent can still narrate those questions in prose — Track 7's layer, untested live. Run 13 then found the mechanism competing with the interviewer mid-conversation and narrowed it (#18). |
| 16 | **Bug** — the gateway's busy acknowledgement reached the organizer ("↪ Redirected current run. I'll adjust using your correction.") | **Fixed** | 2026-09-05, run 13. Not the agent narrating itself (Step 3 #2's class) — the HARNESS narrating itself: an inbound answer arriving while a run was active made the Hermes gateway announce its own scheduling decision as if it were a reply, in English, mid-Hebrew-interview, about a correction the organizer never made. Fixed at the source with `display.busy_ack_enabled: false` in the trip-intake profile (the ack is suppressed; the message is still processed), plus `GATEWAY_STATUS_PHRASES` in `relay/internal-leak.ts` as the fail-safe, since the profile is not version controlled and the ack is on by default. Same class as the `codex_gpt55_autoraise_notice` silencing earlier the same day. |
| 17 | **Bug** — an eleven-minute dead stall: the agent nominated a question, said so, and the question was never drawn | **Fixed** | 2026-09-05, run 13, twice in one hour on `trip_pace`. Read off the agent log and the turn table: `ask_question_for_chat` completed at 23:04:47 (nomination good, phrasing good); the agent's closing text landed at 23:04:50 and became `pendingSay`; `sendNextStep` delivered that say, called `claimFloor` — and **returned**, leaving the nomination behind a floor that now belonged to the organizer. Every later pass logged `floor_held_by_person`. Silence until the organizer typed "מה עכשיו?" eleven minutes later; `trip_pace` was never drawn and never answered, while the closing text had told them it was asked. `nominateQuestionForChat` already folds a say into a nomination, but only one pending when the nomination runs — here the say arrived *after* the ask, which no nomination-time fold can catch. The fold now also happens at render time, where both values come from the same session row in the same pass and the write order stops mattering. |
| 18 | **Bug** — router-owned questions competed with the interviewer mid-conversation | **Fixed** | 2026-09-05, run 13: "it again competing with the agent on the questions." Track 8 fired for any session with `awaiting = 'machine'`, and an open agent turn is equally the state of an agent composing its next message — so a fixed-choice question could land on top of the interviewer. The two cases are now separated by what the turn is *for*: a turn carrying media is real async work (it is the only thing that stamps `awaiting_floor_seconds`) and Track 8 still fills that dead air; any other open turn is a conversation the interviewer owns, and the router yields (`trip_bot.router_owned_yielded`). With no turn open it fires freely, which is the latency half of Track 8 and has nobody to compete with. Directly implements the constraint set when Track 8 was specified — this optimization must not drift the interview back into a form. |
| 19 | **Bug** — a provisioned trip is unroutable because the organizer's full name matches no participant | **Open** | 2026-09-06, found on the first successful provisioning run. `_resolve_organizers` (`transformer.py:649-656`) compares `organizer_identity` against `{name, name_en, username}` only, never `name + family`. The organizer answered "רון מרגולין" against a participant `{name: רון, name_en: Ron, username: ron, family: מרגולין}` — no match, so `agent.organizers` is unset, `build_companion_handoff` returns `None`, the companion is skipped, and `telegram_chat_bindings` stays 0. Worse, the skip logs at `info` and the worker configures no logging, so the whole successful run emitted one line and nothing reports that the trip cannot be reached. Answering with a full name is the normal case, so this is not an edge. |
| 20 | **Bug** — `interview-stack-deploy` does not deploy the worker | **Partial** | 2026-09-06. `deploy.sh` recreates only `api`; the worker's code comes from a bind mount (`WORKER_REPO_ROOT_HOST`) defaulting to a different checkout. Deploying a branch updated the API to intake schema v3 while the worker kept v2's transformer, and provisioning failed three times with `intake is missing required questions: ['group_size', 'trip_duration']` — the exact failure `INTAKE_SCHEMA_VERSION`'s comment predicts, arriving through the one path the guard cannot cover: the two halves came from different trees. Mitigated by consolidating everything onto `integration/sprint-5-plus`, so the default mount is now correct; the script still does not deploy or verify the worker. |
...
| Family name not transliterated (`{he:מרגולין, en:מרגולין}`) | **Partial** | Honoured when the intake supplies `family_en` (done). Auto-transliteration = the LLM pass (General #4 / Sprint 5). |
