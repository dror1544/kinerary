...
47. **#240 (the Orlando companion invented "nut-free options for Eitan") is high and rare, fixed in the template before Release A, and not mitigated on the live companions until their trips end (the lead's call, 2026-09-26; the owner delegated the priority).** Sources: the assessment comment on #240; `docs/test-reports/regression-plan-2026-09-26-release-a-sprint-mode.md` U7.
    ...
    - **Status (2026-09-26): MERGED, not deployed** — PR #245, merge `fb190dd` (two review rounds; round 1 found that a need saved earlier would have stopped counting and that "never name anyone not on the roster" contradicted addressing people by the name they typed; both fixed). The eval never reproduced the echo (0/176 before and after), so **no claim is made that the echo rate fell**; the static test proves the example is gone, and flat allergen claims on the incident-shaped prompt went 6/40 to 0/40. Left, on #246: the audit of other prompts, the lint's known gaps, pinning the eval's mirrored environment to `model-runner.ts`. The two live companions are unchanged until re-rendered after their trips end.
...
50. **A finding about example data in tracked files is recorded in a handover note outside the repository, not here, because the repo is public; the owner chose to redact the tracked tree, and to decide separately whether history needs more.** The redaction is MERGED (PR #258, merge `4726274`); nothing about what it replaced is written in this file on purpose, as with decision 35. History and forks are untouched, and further exposures found while doing it are decided in the handover note, not here. Consequence for #229 (the test reports to `main`): it must be rebuilt from the redacted tree before it goes anywhere near `main`, done: #229 was closed and re-cut as #259 from `main`, one report left out because it is titled with a live family's name and written from their conversations (it stays on the sprint branch). (Dror, 2026-09-26, answering the lead's question)
