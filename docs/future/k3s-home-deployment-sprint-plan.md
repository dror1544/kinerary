...
| C7 | Docs and skills encode LXC + NPM + Cloudflare: smart-home-expert `public-trip-site-ingress*.md` (also TrueNAS `.177` vs the real `.171`, and its "no wildcard" rule), `.agents/skills/trip-site-operations`, `trip-site-verification`, `docs/archived/kinerary-trip-platform-handoff.md`, profile SOULs | D0, D3, D5, D6, D9 | Capture-then-deploy per `CLAUDE.md`; rewrite at D9 | open |
