...
This is the **evaluation** layer: what good looks like, how to score it, and what to report. `trip-bot-analytics-and-metrics-design.md` is the **instrumentation** layer and stays the authority for the base event schema (§5), the request-type taxonomy (§6), bounded Prometheus labels (§8) and retention (§10).

The event types under "Implementation Notes" below are *outcome* events: they describe how well a request was served, where the §6 taxonomy classifies what was asked. They must be emitted inside that base event schema rather than as a parallel system. If the two ever disagree, extend the event contract there and link to it; do not fork a second vocabulary here.

Implementation is scheduled across Sprints 6 and 7 in `onboarding-mvp-sprint-plan.md`.
